...
# --- DEBUG BLOCK (Insert before self.odom_pub_.publish) ---
        # Only print every 20th message to avoi dspamming the terminal
        if msg.header.stamp.nanosec % 200 == 0: 
            self.get_logger().debug(
                f"\n"
                f"--- ODOM DEBUG ---\n"
                f"Deltas  -> Left: {dp_left:.4f}  Right: {dp_right:.4f}\n"
                f"Increm  -> d_s:  {d_s:.4f}    d_th:  {d_theta:.4f}\n"
                f"Pose    -> X:    {self.x_:.4f}    Y:     {self.y_:.4f}    Th: {self.theta_:.4f}\n"
                f"------------------"
            )
