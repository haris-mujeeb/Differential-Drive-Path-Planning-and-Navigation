...
class BFSPlanner(Node):
  ...
  def plan(self, start_pose : Pose, goal_pose : Pose) -> Optional[Path]:
    ...
    if not self.is_free(start_node):
        self.get_logger().warning("Start node is in an obstacle")
        return None
    if not self.is_free(goal_node):
        self.get_logger().warning("Goal node is in an obstacle")
        return None

    q : Deque[GraphNode] = deque()
    q.append(start_node)
    came_from : Dict[GraphNode, Optional[GraphNode]] = { start_node : None } # Dictionary
    ...
    while q:
      ...
      if node == goal_node:
        break
      for dx, dy in dirs:
        next_node = node + (dx, dy)
        if not self.in_bound(next_node): continue
        if next_node in came_from: continue
        if not self.is_free(next_node): continue
    ...
    while current_node is not None:
        pose = self.grid_to_world(current_node)
  ...
  def is_free(self, node : GraphNode):
    return self.map_.data[self.idx(node)] == 0
