...
DijkstraPlanner::DijkstraPlanner() : Node("dijkstra_node")
{
  ...
  goal_pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
    "/goal_pose",
    10,
    std::bind(&DijkstraPlanner::goalPoseCallback, this, std::placeholders::_1)
  );
...
void DijkstraPlanner::goalPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr pose_msg)
{
  if (map_ == nullptr) 
  {
    RCLCPP_ERROR(get_logger(), "No map recieved!");
...
bool DijkstraPlanner::inBound(const GraphNode & node)
{
...
unsigned int DijkstraPlanner::idx(const GraphNode & node)
{
  return  map_->info.width * node.y + node.x;
}

bool DijkstraPlanner::isFree(const GraphNode & node)
{
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<bumperbot_planning::DijkstraPlanner>());
  rclcpp::shutdown();
  return 0;
}